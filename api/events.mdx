...
sequenceDiagram
    ...
    Client->>S3M: POST /s3m/create-multipart-upload
    S3M->>S3: createMultipartUpload()
    S3-->>S3M: uploadId
    S3M->>Events: MultipartUploadCreated
    Events-->>Listeners: Notify
    S3M-->>Client: uuid, bucket, key, uploadId
    
    Client->>S3M: POST /s3m/create-sign-part (per part)
    S3M-->>Client: presigned URL
    Client->>S3: PUT part data
    
    Client->>S3M: POST /s3m/complete-multipart-upload
