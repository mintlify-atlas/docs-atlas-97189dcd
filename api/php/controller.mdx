...
const response = await fetch('/s3m/multipart', {
  method: 'POST',
  headers: {
...
$response = Http::post('/s3m/multipart', [
    'content_type' => 'video/mp4',
    'folder' => 'videos',
    'visibility' => 'private'
]);
...
const signResponse = await fetch('/s3m/multipart/sign', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    'X-CSRF-TOKEN': csrfToken
...
$signResponse = Http::post('/s3m/multipart/sign', [
    'key' => $key,
    'upload_id' => $uploadId,
    'part_number' => 1,
    'content_type' => 'video/mp4'
...
<CodeGroup>
```javascript JavaScript
const completeResponse = await fetch('/s3m/multipart/complete', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    'X-CSRF-TOKEN': csrfToken
...
$response = Http::post('/s3m/multipart/complete', [
    'key' => $key,
    'upload_id' => $uploadId,
    'parts' => [
        ['PartNumber' => 1, 'ETag' => '"etag-from-part-1"'],
